...
describe("Check if cache updated (E2E)", () => {
  ...
  it("Generate one file, then change generated again and regenerate, target file should be untouched due to cache", async () => {
    fakeFs.addFile(
      "svi.json",
      `
      {
...
`,
    ...
    fakeFs.changeFileContent(
      "test.js",
      `
Updated JS File Content
`,
    );
    ...
    const updatedContent = fakeFs.fileContent("test.js");

    expect(updatedContent).toContain("Updated JS File Content");
  });
});
